...
import { addCartItem, createCart, createOrder, fetchCustomerOrders, getCart, getOrCreateCustomer, markPaymentFailed, removeCartItem, searchShop, updateCartItem, verifyPayment } from "@/lib/api";
...
function Shop() {
  ...
  async function changeQty(itemId: number, delta: number) {
    ...
    try {
      const result = await updateCartItem(cart.id, itemId, newQty);
      setCart(result.cart);
    } catch {
          ...
          {recommendation ? (
            <article className="border border-border bg-card p-6 shadow-[var(--shadow-panel)]">
              ...
              <div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-center">
                <div className="flex size-32 shrink-0 items-center justify-center overflow-hidden rounded-md bg-muted text-xs text-muted-foreground"><img src={recommendation.image_url || ""} alt={recommendation.name} className="h-full w-full object-cover" onError={(e) => { e.currentTarget.style.display = "none"; const s = e.currentTarget.nextElementSibling as HTMLElement | null; if (s) s.style.display = "flex"; }} onLoad={(e) => { const s = e.currentTarget.nextElementSibling as HTMLElement | null; if (s) s.style.display = "none"; }} /><span style={{ display: "none" }} className="m-auto">Image unavailable</span></div>
